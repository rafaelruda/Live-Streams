class PeerBuilder {
  constructor({ peerCofig }) {
    this.peerCofig = peerCofig;
  ...
  _preparePeerInstanceFunction(peerModule) {
    ...
    PeerCustomModule.prototype.call = function (id, stream) {
      ...
      // aqui acontece a intercepatação do call e adicionamos
  ...
  build() {
    // const peer = new Peer(...this.peerCofig);
    const PeerCustomInstance = this._preparePeerInstanceFunction(Peer);
    const peer = new PeerCustomInstance(...this.peerCofig);
